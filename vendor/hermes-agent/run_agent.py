...
class AIAgent:
    ...
    def __init__(
        ...
        self.extra_body_override = extra_body_override or {}  # Merged into every API call's extra_body
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # ── Qwen XML tool call recovery (streaming path) ──
            # Qwen outputs tool calls as <tool_call> XML in content.
            # Parse them here before the mock response is built.
            if not mock_tool_calls and full_content and "<tool_call>" in full_content:
                import re as _re_s
                _TC = _re_s.compile(r"<tool_call>\s*<function=(\w+)>(.*?)</function>\s*</tool_call>", _re_s.DOTALL)
                _PM = _re_s.compile(r"<parameter=(\w+)>\s*(.*?)\s*</parameter>", _re_s.DOTALL)
                _stcs = []
                for _m in _TC.finditer(full_content):
                    _a = {}
                    for _p in _PM.finditer(_m.group(2)):
                        _v = _p.group(2).strip()
                        try: _v = json.loads(_v)
                        except (json.JSONDecodeError, ValueError): pass
                        _a[_p.group(1)] = _v
                    _stcs.append(SimpleNamespace(
                        id=f"qwen_stream_tc_{len(_stcs)}", type="function",
                        function=SimpleNamespace(name=_m.group(1), arguments=json.dumps(_a, ensure_ascii=False)),
                    ))
                if _stcs:
                    mock_tool_calls = _stcs
                    _first = full_content.find("<tool_call>")
                    _thinking = _re_s.sub(r"</think>\s*$", "", full_content[:_first]).strip() if _first > 0 else ""
                    full_content = _thinking or None
                    finish_reason = "tool_calls"
                    logger.info("Qwen streaming: parsed %d XML tool call(s)", len(_stcs))
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        # Merge caller-supplied extra_body fields (e.g. vLLM chat_template_kwargs)
        if self.extra_body_override:
            extra_body.update(self.extra_body_override)
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    # Qwen 3.5 via vLLM: streaming mode breaks XML tool calls
                    # (content comes empty, tool_call XML lost). Force non-streaming
                    # for local/custom endpoints with Qwen models.
                    _model_lower = (self.model or "").lower()
                    if "qwen" in _model_lower and self.platform == "api":
                        _use_streaming = False
            ...
            try:
                ...
                # ── Qwen XML tool call & thinking recovery ──
                # Qwen 3.5 outputs:
                #   1. Inline reasoning (thinking) as regular text
                #   2. Optional </think> closing tag (opening <think> stripped by vLLM)
                #   3. Tool calls as <tool_call><function=name>...</tool_call> XML
                # None of this comes through structured tool_calls or reasoning_content.
                # We parse it here and set the proper fields so Hermes can use them.
                if (not assistant_message.tool_calls
                        and assistant_message.content
                        and isinstance(assistant_message.content, str)
                        and "<tool_call>" in assistant_message.content):
                    import re as _re_qwen
                    _TC_RE = _re_qwen.compile(
                        r"<tool_call>\s*<function=(\w+)>(.*?)</function>\s*</tool_call>",
                        _re_qwen.DOTALL,
                    )
                    _PARAM_RE = _re_qwen.compile(
                        r"<parameter=(\w+)>\s*(.*?)\s*</parameter>",
                        _re_qwen.DOTALL,
                    )
                    _parsed_tcs = []
                    for _m in _TC_RE.finditer(assistant_message.content):
                        _fn_name = _m.group(1)
                        _body = _m.group(2)
                        _args = {}
                        for _pm in _PARAM_RE.finditer(_body):
                            _pname = _pm.group(1)
                            _pval = _pm.group(2).strip()
                            try:
                                _pval = json.loads(_pval)
                            except (json.JSONDecodeError, ValueError):
                                pass
                            _args[_pname] = _pval
                        _tc_id = f"qwen_tc_{len(_parsed_tcs)}"
                        _parsed_tcs.append(SimpleNamespace(
                            id=_tc_id,
                            type="function",
                            function=SimpleNamespace(
                                name=_fn_name,
                    ...
                    if _parsed_tcs:
                        # Extract thinking: everything before the first <tool_call>
                        _raw = assistant_message.content
                        _first_tc = _raw.find("<tool_call>")
                        _thinking = _raw[:_first_tc].strip() if _first_tc > 0 else ""
                        # Strip orphan </think> tag (vLLM strips opening <think>)
                        _thinking = _re_qwen.sub(r"</think>\s*$", "", _thinking).strip()

                        # Set structured tool calls
                        assistant_message.tool_calls = _parsed_tcs
                        # Keep thinking as content — Hermes logs it and includes
                        # it in conversation history for context continuity
                        assistant_message.content = _thinking or None
                        # Also set reasoning_content so Hermes's reasoning
                        # callback and display can use it
                        if _thinking and not getattr(assistant_message, "reasoning_content", None):
                            assistant_message.reasoning_content = _thinking

                        logger.info("Qwen: parsed %d tool call(s), thinking=%d chars",
                                    len(_parsed_tcs), len(_thinking))
